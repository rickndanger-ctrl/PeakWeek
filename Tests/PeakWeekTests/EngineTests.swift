...
final class EngineTests: XCTestCase {
    ...
    func testFullPrepAllLengthsSumAndOrder() {
        for n in 10...16 {
            let p = Engine.buildProgram(startPhase: .full, totalWeeks: n, fiveDay: false, library: lib)
    ...
    func testOffSeasonLayout() {
        let p = Engine.buildProgram(startPhase: .offseason, totalWeeks: 10, fiveDay: false, library: lib)
    ...
    func testSingleBlockPrograms() {
        let acc = Engine.buildProgram(startPhase: .acc, totalWeeks: 5, fiveDay: false, library: lib)
        XCTAssertEqual(acc.blocks.map(\.phase), [.acc])
        XCTAssertEqual(acc.weeks.count, 5)
        XCTAssertFalse(acc.weeks.contains { $0.phase == .meet })

        let real = Engine.buildProgram(startPhase: .real, totalWeeks: 3, fiveDay: false, library: lib)
    ...
    func testWeek1Day1CompetitionSquat() {
        ...
        XCTAssertEqual(Engine.slotName(s, library: lib), "Competition Squat")
        XCTAssertEqual(s.sets, 4)
        XCTAssertEqual(s.reps, 6)
        XCTAssertEqual(s.pct, 67)
        XCTAssertEqual(Engine.slotLoad(s, maxes: maxes, unit: .lb, library: lib), 270)
    }
    ...
    func testVariationLoadUsesModifier() {
        ...
        XCTAssertEqual(Engine.slotLoad(s, maxes: maxes, unit: .lb, library: lib), 245)
    }

    func testAccessoryAndCustomSlotsHaveNoLoad() {
        let accessory = Slot(pool: .quads, exIdx: 0, sets: 3, reps: 10, pct: nil, rpe: 8)
        XCTAssertNil(Engine.slotLoad(accessory, maxes: maxes, unit: .lb, library: lib))
        var custom = Slot(pool: .squat, exIdx: 0, sets: 3, reps: 5, pct: 75, rpe: 8)
        custom.custom = "Belt Squat"
        XCTAssertNil(Engine.slotLoad(custom, maxes: maxes, unit: .lb, library: lib), "custom exercises have no computed load")
        XCTAssertEqual(Engine.slotName(custom, library: lib), "Belt Squat")
    }

    func testZeroMaxProducesNoLoad() {
        let s = Slot(pool: .squat, exIdx: 0, sets: 4, reps: 6, pct: 67, rpe: 7)
        XCTAssertNil(Engine.slotLoad(s, maxes: Maxes(), unit: .lb, library: lib))
    }
    ...
    func testFiveDayOnlyAddsToAccAndTrans() {
        let p = Engine.buildProgram(startPhase: .full, totalWeeks: 12, fiveDay: true, library: lib)
        for w in p.weeks {
            let expected: Int
            switch w.phase {
    ...
    func testRealizationTaperStructure() {
        ...
        let p = Engine.buildProgram(startPhase: .full, totalWeeks: 14, fiveDay: false, library: lib)
    ...
    func testMeetWeekExportIncludesAttempts() {
        let p = Engine.buildProgram(startPhase: .full, totalWeeks: 12, fiveDay: false, library: lib)
        let text = Engine.weekToText(client: client, program: p, week: p.weeks.last!, library: lib)
    ...
    func testCoachNotesAppearInExport() {
        ...
        let text = Engine.weekToText(client: client, program: p, week: week, library: lib)
    ...
    func testWeekPDFRenders() {
        ...
        for week in [p.weeks.first!, p.weeks.last!] {
            let data = WeekExporter.pdfData(client: client, program: p, week: week, library: lib)
    ...
    func testCodableRoundTrip() throws {
        var c = client
        c.program = Engine.buildProgram(startPhase: .full, totalWeeks: 12, fiveDay: true, library: lib)
        let enc = JSONEncoder()
